...
- [ ] CIV-003 - Map release artifacts to immutable source commits
  ...
  - Progress: Structurize, BlockUI, Domum Ornamentum, Multi-Piston, Waystones, and Balm now map to immutable source commits/tags in `docs/ARTIFACT_AUDIT.md`. Immersive EGO `0.1.0-alpha.29` binary parity is proven against the sibling build artifact, but the sibling source checkout is dirty.
  - Blocker: Immersive EGO `0.1.0-alpha.29` must be committed/pushed or restored to the previously mapped alpha.27 source state. Modern Companions CurseForge `2.0` artifact is verified, but no checked public `STRHercules/ModernCompanions` branch declares `2.0`, public branch heads predate the April 10, 2026 CurseForge upload, public tags stop at `v1.2.0`, GitHub/Modrinth public releases stop at `v1.2.0`, and `MajorBonghits/ModernCompanions` returned `404`.
...
- [ ] CIV-058 - Recruit MCA villager into MineColonies colony
  ...
  - Evidence: `docs/MINECOLONIES_ASSIGNMENT_API_AUDIT.md`; `build\gametest-smoke.log` reported `All 5 required tests passed :)`, including the upstream resident API and MineColonies assignment API contracts, with Immersive EGO `0.1.0-alpha.29`; `build\install-report.json` installed `immersive_ego_civitas-0.1.0-alpha.14.jar` with SHA-256 `9b237d04a53e50de9358e3c31456e49c42c5d3372da7d44b9e9583e9ee6fa495`; `build\client-smoke-report.json` reported `result=passed` with a responsive `Minecraft NeoForge* 1.21.1` client and no crash markers.

## Blockers

- Immersive EGO `0.1.0-alpha.29` source mapping is local-binary-only because
