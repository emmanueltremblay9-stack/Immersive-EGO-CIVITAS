...
- [ ] CIV-003 - Map release artifacts to immutable source commits
  ...
  - Progress: Structurize, BlockUI, Domum Ornamentum, Multi-Piston, Waystones, and Balm now map to immutable source commits/tags in `docs/ARTIFACT_AUDIT.md`. Immersive EGO `0.1.0-alpha.38` binary parity is proven against the sibling build artifact, but the sibling source checkout is dirty.
  - Blocker: Immersive EGO `0.1.0-alpha.38` must be committed/pushed or restored to the previously mapped alpha.27 source state. Modern Companions CurseForge `2.0` artifact is verified, but no checked public `STRHercules/ModernCompanions` branch declares `2.0`, public branch heads predate the April 10, 2026 CurseForge upload, public tags stop at `v1.2.0`, GitHub/Modrinth public releases stop at `v1.2.0`, and `MajorBonghits/ModernCompanions` returned `404`.
...
- [ ] CIV-058 - Recruit MCA villager into MineColonies colony
  ...
  - Progress: Original CIVITAS canonical resident identity scaffold is complete: one resident per external host key, reverse host index, cross-resident host collision guard, `SavedData` persistence, neutral host-adapter registry/service abstractions, verified reflection-backed MCA/MineColonies resident host adapters, and original MCA-to-MineColonies resident recruitment orchestration that links exact verified host keys into one record. The exact public MineColonies assignment API surface is proven by source/runtime audit and a GameTest contract. Alpha.14 adds a repairable assignment executor that routes through MineColonies assignment modules and rolls back target module changes on downstream assignment failures, including partial home/work mutations that throw. Alpha.18 adds live building-object module discovery and a coordinator that routes discovered home/work modules through the repairable executor. Alpha.21 adds a linked-resident trigger service that refuses to call MineColonies assignment unless the citizen already maps to a CIVITAS record with both MineColonies and MCA host links. Alpha.23 adds operator-only assignment commands that resolve live MineColonies colony, citizen, home building, and work building targets, verify building ownership, then route through the linked-resident guard and repairable assignment coordinator. Alpha.25 adds `/civitas link_mca_minecolonies`, which links a selected MCA villager entity and a resolved MineColonies citizen into persisted CIVITAS saved data before assignment.
  ...
  - Evidence: `docs/MINECOLONIES_ASSIGNMENT_API_AUDIT.md`; `build\gametest-smoke.log` reported `All 7 required tests passed :)`, including the upstream resident API, MineColonies assignment API contract, locator runtime type contract, linked-resident assignment trigger guard, and persisted recruitment saved-data GameTest, with Immersive EGO `0.1.0-alpha.38`; `build\install-report.json` installed `immersive_ego_civitas-0.1.0-alpha.25.jar`; `build\client-smoke-report.json` reported `result=passed` with a responsive `Minecraft NeoForge* 1.21.1` client and no crash markers.

## Blockers

- Immersive EGO `0.1.0-alpha.38` source mapping is local-binary-only because
