...
|---|---|---|---|---|
| CIVITAS | 1.21.1 | NeoForge | 0.1.0-alpha.21 | Harness builds, installs, and passes server/client smoke with pinned runtime guard, resident SavedData GameTests, verified reflection-backed MCA/MineColonies resident host adapter coverage, original resident recruitment orchestration unit coverage, a MineColonies assignment API contract GameTest, repairable assignment executor unit coverage, live assignment module locator/coordinator unit coverage, and linked-resident assignment trigger coverage |
| Immersive EGO | 1.21.1 | NeoForge | 0.1.0-alpha.35 | LAB prerequisite loaded in server/client smoke; installed jar hash matches the sibling build artifact, but the sibling checkout has uncommitted changes so immutable source mapping is blocked |
