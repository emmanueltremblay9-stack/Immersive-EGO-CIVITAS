...
|---|---|---|---|---|
| CIVITAS | 1.21.1 | NeoForge | 0.1.0-alpha.25 | Harness builds, installs, and passes server/client smoke with pinned runtime guard, resident SavedData GameTests, verified reflection-backed MCA/MineColonies resident host adapter coverage, original resident recruitment orchestration unit coverage, persisted saved-data recruitment GameTest coverage, a MineColonies assignment API contract GameTest, repairable assignment executor unit coverage, live assignment module locator/coordinator unit coverage, linked-resident assignment trigger coverage, operator link command wiring, and operator assignment command lookup coverage |
| Immersive EGO | 1.21.1 | NeoForge | 0.1.0-alpha.38 | LAB prerequisite loaded in server/client smoke; installed jar hash matches the sibling build artifact, but the sibling checkout has uncommitted changes so immutable source mapping is blocked |
