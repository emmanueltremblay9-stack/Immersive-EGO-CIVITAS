...
- Inspected pinned MineColonies source/runtime assignment surfaces and added
  original `MineColoniesAssignmentApiContract` coverage for the public home,
  work, assignment-module, and job methods that CIVITAS must use before
  mutating colony state.
- Added `docs/MINECOLONIES_ASSIGNMENT_API_AUDIT.md` and a GameTest asserting
  that the installed MineColonies runtime still exposes that assignment API
  surface.
- Built and installed `immersive_ego_civitas-0.1.0-alpha.10.jar` into the Prism
  LAB `minecraft\mods` folder with SHA-256 match.
- Added original repairable MineColonies assignment execution:
  `MineColoniesAssignmentPlan`, `MineColoniesAssignmentResult`, and
  `MineColoniesAssignmentService`. The executor calls assignment modules
  reflectively and rolls back target module changes on downstream assignment
  failures without directly setting MineColonies citizen fields.
- Added original MineColonies assignment module discovery:
  ...
  building-object home/work assignment modules through the verified public
  `IAssignsCitizen` and `IAssignsJob` interfaces, rejects ambiguous or full
  targets, captures previous modules for rollback, and routes resolved plans
  through `MineColoniesAssignmentService`.
- Refreshed the exact CIVITAS runtime guard to accept the current Prism LAB
  Immersive EGO prerequisite `0.1.0-alpha.32`. The installed
  `immersive_ego-0.1.0-alpha.32.jar` hash matches the sibling build artifact,
  ...
  restored.
- Built and installed `immersive_ego_civitas-0.1.0-alpha.18.jar` into the Prism
  LAB `minecraft\mods` folder with SHA-256 match.
- Refreshed the exact CIVITAS runtime guard to accept the current Prism LAB
  Immersive EGO prerequisite `0.1.0-alpha.33`. The installed
  `immersive_ego-0.1.0-alpha.33.jar` hash matches the sibling build artifact,
  but the sibling source checkout has uncommitted changes, so immutable source
  mapping is blocked until that source state is committed/pushed or alpha.27 is
  restored.
- Built and installed `immersive_ego_civitas-0.1.0-alpha.19.jar` into the Prism
  LAB `minecraft\mods` folder with SHA-256 match.
- Added original linked-resident assignment trigger guards:
  `LinkedResidentAssignmentService`, `ResidentDirectory`, and
  `MineColoniesAssignmentGateway`. The trigger refuses to call the
  MineColonies assignment gateway unless the MineColonies citizen resolves to
  an existing CIVITAS resident that also has an MCA Reborn host link.
- Refreshed the exact CIVITAS runtime guard to accept the current Prism LAB
  Immersive EGO prerequisite `0.1.0-alpha.35`. The installed
  `immersive_ego-0.1.0-alpha.35.jar` hash matches the sibling build artifact,
  but the sibling source checkout has uncommitted changes, so immutable source
  mapping is blocked until that source state is committed/pushed or alpha.27 is
  restored.
...
- Built and installed `immersive_ego_civitas-0.1.0-alpha.23.jar` into the Prism
  LAB `minecraft\mods` folder with SHA-256 match.
- Added original persisted resident link command wiring:
  ...
  persisted `CivitasResidentSavedData`.
- Added `ResidentStore` so `ResidentRecruitmentService` can operate on runtime
  saved data, and added `MineColoniesCitizenTarget` for citizen-only
  MineColonies lookup.
- Refreshed the exact CIVITAS runtime guard to accept the current Prism LAB
  Immersive EGO prerequisite `0.1.0-alpha.38`. The installed
  `immersive_ego-0.1.0-alpha.38.jar` hash matches the sibling build artifact,
  but the sibling source checkout has uncommitted changes, so immutable source
  mapping is blocked until that source state is committed/pushed or alpha.27 is
  restored. Alpha.24 failed server smoke only because the LAB prerequisite had
  already drifted past its alpha.37 pin; alpha.25 supersedes it.
- Built and installed `immersive_ego_civitas-0.1.0-alpha.25.jar` into the Prism
  LAB `minecraft\mods` folder with SHA-256 match.
...
Invoke-WebRequest -UseBasicParsing -Uri 'https://codeload.github.com/ldtteam/minecolonies/zip/35bd7ad7448c562c84d11dc9dff5b067e8f131e5'
javap -classpath "C:\Users\Emmanuel Tremblay\AppData\Roaming\PrismLauncher\instances\1.21.1 TesT LaB\minecraft\mods\minecolonies-1.1.1319-1.21.1.jar" -public com.minecolonies.api.colony.buildings.modules.IAssignsCitizen com.minecolonies.api.colony.buildings.modules.IAssignsJob
javap -classpath "C:\Users\Emmanuel Tremblay\AppData\Roaming\PrismLauncher\instances\1.21.1 TesT LaB\minecraft\mods\minecolonies-1.1.1319-1.21.1.jar" -public com.minecolonies.api.colony.ICitizenData com.minecolonies.api.colony.jobs.IJob
javap -classpath "C:\Users\Emmanuel Tremblay\AppData\Roaming\PrismLauncher\instances\1.21.1 TesT LaB\minecraft\mods\minecolonies-1.1.1319-1.21.1.jar" -public com.minecolonies.core.colony.jobs.AbstractJob
.\gradlew.bat --no-daemon test
.\gradlew.bat --no-daemon clean build
pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\validate-provenance.ps1
pwsh -NoProfile -ExecutionPolicy Bypass -File .\install-mod.ps1 -SkipBuild
pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\run-gametest-smoke.ps1
pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\run-prism-client-smoke.ps1 -TimeoutSeconds 240
jar tf "C:\Users\Emmanuel Tremblay\AppData\Roaming\PrismLauncher\instances\1.21.1 TesT LaB\minecraft\mods\immersive_ego_civitas-0.1.0-alpha.10.jar"
.\gradlew.bat --no-daemon test
.\gradlew.bat --no-daemon clean build
pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\validate-provenance.ps1
pwsh -NoProfile -ExecutionPolicy Bypass -File .\install-mod.ps1 -SkipBuild
pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\run-gametest-smoke.ps1
pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\run-prism-client-smoke.ps1 -TimeoutSeconds 240
jar tf "C:\Users\Emmanuel Tremblay\AppData\Roaming\PrismLauncher\instances\1.21.1 TesT LaB\minecraft\mods\immersive_ego_civitas-0.1.0-alpha.11.jar"
Get-ChildItem 'C:\Users\Emmanuel Tremblay\AppData\Roaming\PrismLauncher\instances\1.21.1 TesT LaB\minecraft\mods' -Filter 'immersive_ego*.jar'
Get-FileHash -Algorithm SHA256 -LiteralPath 'C:\Users\Emmanuel Tremblay\AppData\Roaming\PrismLauncher\instances\1.21.1 TesT LaB\minecraft\mods\immersive_ego-0.1.0-alpha.32.jar'
git status --short --branch
.\gradlew.bat --no-daemon test
.\gradlew.bat --no-daemon clean build
pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\validate-provenance.ps1
...
pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\run-gametest-smoke.ps1
pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\run-prism-client-smoke.ps1 -TimeoutSeconds 240
jar tf "C:\Users\Emmanuel Tremblay\AppData\Roaming\PrismLauncher\instances\1.21.1 TesT LaB\minecraft\mods\immersive_ego_civitas-0.1.0-alpha.18.jar"
Get-FileHash -Algorithm SHA256 -LiteralPath 'C:\Users\Emmanuel Tremblay\AppData\Roaming\PrismLauncher\instances\1.21.1 TesT LaB\minecraft\mods\immersive_ego-0.1.0-alpha.33.jar'
.\gradlew.bat --no-daemon test
.\gradlew.bat --no-daemon clean build
pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\validate-provenance.ps1
pwsh -NoProfile -ExecutionPolicy Bypass -File .\install-mod.ps1 -SkipBuild
pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\run-gametest-smoke.ps1
pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\run-prism-client-smoke.ps1 -TimeoutSeconds 240
jar tf "C:\Users\Emmanuel Tremblay\AppData\Roaming\PrismLauncher\instances\1.21.1 TesT LaB\minecraft\mods\immersive_ego_civitas-0.1.0-alpha.19.jar"
Get-FileHash -Algorithm SHA256 -LiteralPath 'C:\Users\Emmanuel Tremblay\AppData\Roaming\PrismLauncher\instances\1.21.1 TesT LaB\minecraft\mods\immersive_ego-0.1.0-alpha.35.jar'
.\gradlew.bat --no-daemon test
.\gradlew.bat --no-daemon clean build
pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\validate-provenance.ps1
pwsh -NoProfile -ExecutionPolicy Bypass -File .\install-mod.ps1 -SkipBuild
...
Get-FileHash -Algorithm SHA256 -LiteralPath 'C:\Users\Emmanuel Tremblay\AppData\Roaming\PrismLauncher\instances\1.21.1 TesT LaB\minecraft\mods\immersive_ego-0.1.0-alpha.38.jar'
Get-FileHash -Algorithm SHA256 -LiteralPath 'C:\Users\Emmanuel Tremblay\AI Depot\Codex Documents\Immersive EGO\build\libs\immersive_ego-0.1.0-alpha.38.jar'
.\gradlew.bat --no-daemon test
.\gradlew.bat --no-daemon clean build
pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\validate-provenance.ps1
pwsh -NoProfile -ExecutionPolicy Bypass -File .\install-mod.ps1 -SkipBuild
pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\run-gametest-smoke.ps1
pwsh -NoProfile -ExecutionPolicy Bypass -File scripts\run-prism-client-smoke.ps1 -TimeoutSeconds 240
jar tf "C:\Users\Emmanuel Tremblay\AppData\Roaming\PrismLauncher\instances\1.21.1 TesT LaB\minecraft\mods\immersive_ego_civitas-0.1.0-alpha.25.jar"
```
...
- `scripts\run-gametest-smoke.ps1` passed and `build\gametest-smoke.log`
  contains `All 7 required tests passed :)` and
...
- `build\local-runtime-stage-report.json` staged Immersive EGO
  `0.1.0-alpha.38` from the Prism LAB mods directory with SHA-256
  `2e37d092018f86e7f92ba7bf30f6d9cbaca0d2e93c5b3385b8ad1b88fae5a37e`.
- The installed alpha.25 jar contains the resident upstream adapter classes,
  ...
  `MineColoniesAssignmentTargetResolver.class`,
  `MineColoniesCitizenTarget.class`, `CivitasServerCommands.class`, and
...
- Sibling Immersive EGO build artifact
  `build\libs\immersive_ego-0.1.0-alpha.38.jar` matches the installed Prism
  LAB prerequisite SHA-256
  `2e37d092018f86e7f92ba7bf30f6d9cbaca0d2e93c5b3385b8ad1b88fae5a37e`, but
  the sibling repository is dirty.
- The sibling Immersive EGO GameTest proof was not rerun for dirty
  `0.1.0-alpha.38` in this CIVITAS pass; the earlier 3-test proof belongs to
...
- None. Only original CIVITAS bootstrap, runtime guard, resident registry,
  SavedData, persisted store abstraction, host-adapter registry, identity service, reflection-backed
  ...
  resolver, citizen-only target lookup, operator command wiring, and test
...
- Immersive EGO `0.1.0-alpha.38` source mapping is blocked until the dirty
...
- Immersive EGO `0.1.0-alpha.38` has local binary parity only because the
...
Continue `CIV-058` by executing or automating a full live vertical slice:
select a real MCA villager, link it to a real MineColonies citizen with
`/civitas link_mca_minecolonies`, then apply home/work assignment commands to
real MineColonies buildings and verify colony state. Modern Companions `2.0`
source mapping and the dirty Immersive EGO `0.1.0-alpha.38` source state remain
