...
| Project | Code license | Asset status | Copy code? | Copy assets? | Current action |
|---|---|---|---|---|---|
| Immersive EGO: CIVITAS | GPL-3.0-only | Original CIVITAS assets only | N/A | Original only | Keep corresponding source and notices |
| NeoForge | LGPL-2.1-only | Upstream assets not copied | No | No | External dependency only |
| Immersive EGO | MIT | Separate project assets not copied | Prefer API only | No | Keep GPL code out of MIT repo |
| MineColonies | GPL-3.0 | Assets require separate audit | Yes, with provenance | No by default | Map release files before adaptation |
| BlockUI | GPL3 in jar metadata; GPL-3.0 from repository API | Not copied | No by default | No | External dependency |
...
| Basic Weapons | Pending lineage audit | Assets not approved | Blocked | No | Audit before adaptation |

No third-party assets are currently approved for copying.
